...
from outmajor_credits_calculate import validate_software_engineering_requirements
app = Flask(__name__)
...
def check_missing_courses(data):
    ...
    with engine.connect() as connection:
        ...
        for course_type in course_types:
            # 根据课程类别选择对应的表格(必修和专业核心)
            if "required" in course_type:
            ...
            elif "outmajor" in course_type:
                outmajor_result = validate_software_engineering_requirements(data)

                # 检查返回值是否为字符串（错误信息）
                if isinstance(outmajor_result, str):
                    missing_courses_result.setdefault('软工商务类课程', []).append(outmajor_result)
                elif isinstance(outmajor_result, list):
                    missing_courses_result.setdefault('软工商务类课程', []).extend(outmajor_result)
